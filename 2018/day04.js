...
for (const line of input.split('\n')) {
    ...
    let [timestamp, message] = line.split('] ');
    timestamp = timestamp.substring(1); // remove '['

    const [day, time] = timestamp.split(' ');
    const minute = parseInt(time.split(':')[1]);

    log.push({timestamp, day, minute, message});
}
...
for (const logEntry of log) {
    if (logEntry.message.startsWith('Guard #')) {
        const guardId = parseInt(logEntry.message.split('#')[1].split(' ')[0]);
        curGuard = guards[guardId];
        if (!curGuard) {
            curGuard = { id: guardId, minutes: new Array(60).fill(0) };
            guards[guardId] = curGuard;
        }
    } else if (logEntry.message.startsWith('falls asleep')) {
        sleepsSince = logEntry.minute;
    } else if (logEntry.message.startsWith('wakes up')) {
        console.log('sleeping until ', logEntry.minute);
        for (let i = sleepsSince; i < logEntry.minute; ++i) {
            ++curGuard.minutes[i];
        }
    }
}

const aggregations = Object.values(guards)
    .map(guard => {
        const favoriteMinuteCount = Math.max(...guard.minutes);
        const favoriteMinute = guard.minutes.indexOf(favoriteMinuteCount);
        const totalMinutes = sum(guard.minutes);
        return {
            guard,
            favoriteMinute,
            favoriteMinuteCount,
...
const sleepiestGuard = [...aggregations].sort((a, b) => b.totalMinutes - a.totalMinutes)[0];
console.log(`Guard that sleeps most: ${sleepiestGuard.guard.id} (${sleepiestGuard.totalMinutes} minutes)`);
console.log(`Guard ${sleepiestGuard.guard.id} sleeps most often in minute ${sleepiestGuard.favoriteMinute} => solution=${sleepiestGuard.guard.id * sleepiestGuard.favoriteMinute}`);


// Part 2

const highestCountGuard = [...aggregations].sort((a, b) => b.favoriteMinuteCount - a.favoriteMinuteCount)[0];
console.log(`Guard ${highestCountGuard.guard.id} has highest count of ${highestCountGuard.favoriteMinuteCount} on minute ${highestCountGuard.favoriteMinute} => solution=${highestCountGuard.guard.id * highestCountGuard.favoriteMinute}`);