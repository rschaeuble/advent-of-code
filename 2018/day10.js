...
const items = [];
const re = new RegExp('position=<(.+), (.+)> velocity=<(.+), (.+)>');
...
function area() {
    const [minX, minY, maxX, maxY] = boudingBox();
    return (maxX - minX + 1) * (maxY - minY + 1);
}

function printItems() {
    const [minX, minY, maxX, maxY] = boudingBox();
    const buffer = new Array(maxY - minY + 1).fill().map(_ => new Array(maxX - minX + 1).fill('.'));

    for (const item of items) {
        buffer[item.y - minY][item.x - minX] = '#';
    }

    console.log(buffer.map(line => line.join('')).join('\n'));
}

function moveItems(forward = true) {
